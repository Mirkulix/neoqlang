    ...
    <div className="card provider-card" style={{ opacity: provider.enabled ? 1 : 0.6 }}>
      ...
      {(provider.requests ?? 0) > 0 && (
        <div className="provider-stats-grid">
          <div>
            <div className="label">Requests</div>
            <div className="stat-value" style={{ fontSize: '16px', color: 'var(--accent-primary)' }}>{provider.requests}</div>
          </div>
          <div>
            <div className="label">Tokens</div>
            <div className="stat-value" style={{ fontSize: '16px', color: 'var(--accent-primary)' }}>{fmtTokens(provider.tokens ?? 0)}</div>
          </div>
          <div>
            <div className="label">Kosten</div>
            <div className="stat-value mono" style={{ fontSize: '16px' }}>{fmtCost(provider.cost_usd ?? 0)}</div>
          </div>
          <div>
            <div className="label">Latenz</div>
            <div className="stat-value mono" style={{ fontSize: '16px' }}>{(provider.avg_latency_ms ?? 0) > 0 ? `${provider.avg_latency_ms}ms` : '—'}</div>
          </div>
        </div>
      )}
      ...
      {provider.source === 'env' && (
        <div style={{ fontSize: '11px', color: 'var(--text-muted)', fontStyle: 'italic' }}>
          Quelle: Umgebungsvariable — Bearbeiten speichert in DB
        </div>
      )}
